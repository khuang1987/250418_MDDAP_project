...
def convert_and_merge_standard_time(routing_csv: str, machining_csv: str, output_path: str, cfg: Dict[str, Any] = None) -> None:
    """
    转换并合并两个标准时间表
    1. 读取Routing表和机加工清单表
...
if __name__ == "__main__":
    ...
    # 确定CSV文件路径
    csv_base_dir = cfg.get("source", {}).get("csv_base_dir", os.path.join(BASE_DIR, "..", "11数据模板"))
    csv_base_dir = os.path.abspath(csv_base_dir)
    
    routing_csv = os.path.join(csv_base_dir, cfg.get("source", {}).get("routing_csv", "1303 Routing及机加工产品清单_1303 Routing.csv"))
    machining_csv = os.path.join(csv_base_dir, cfg.get("source", {}).get("machining_csv", "1303 Routing及机加工产品清单_1303机加工清单.csv"))
    ...
    try:
        ...
        # 转换并合并两个表
        convert_and_merge_standard_time(routing_csv, machining_csv, merged_parquet, cfg)
