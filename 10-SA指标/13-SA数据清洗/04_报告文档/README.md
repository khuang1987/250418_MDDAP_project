...
```
13-SA数据清洗/
├── etl_dataclean_mes_batch_report.py    # MES数据批次报工表ETL脚本
├── etl_dataclean_sfc_batch_report.py    # SFC数据批次报工表ETL脚本
├── etl_utils.py                         # ETL通用工具函数模块
├── config/
│   ├── config_mes_batch_report.yaml     # MES批次报工表配置文件
│   └── config_sfc_batch_report.yaml     # SFC批次报工表配置文件
├── logs/                                 # 日志文件
├── publish/                              # 输出文件（Parquet格式）
├── requirements.txt                      # Python依赖
├── run_etl.bat                           # 运行MES批次报工脚本（单独）
...
为便于维护和扩展，所有文件遵循统一的命名规范：

#### ETL脚本文件
**格式：** `etl_{功能}_{数据源}_{表类型}.py`

**示例：**
- `etl_dataclean_mes_batch_report.py` - 数据清洗-MES-批次报工表
- `etl_dataclean_sfc_batch_report.py` - 数据清洗-SFC-批次报工表

**说明：**
- `功能`：dataclean（数据清洗）、transform（转换）、validate（验证）等
- `数据源`：mes、sfc、sap、standard_time等
- `表类型`：batch_report（批次报工）、inspection（检验）、quality（质量）等

#### 配置文件
**格式：** `config_{数据源}_{表类型}.yaml`

**示例：**
- `config_mes_batch_report.yaml` - MES批次报工表配置
- `config_sfc_batch_report.yaml` - SFC批次报工表配置

#### 对应关系
```
etl_dataclean_{数据源}_{表类型}.py  ←→  config_{数据源}_{表类型}.yaml
```

### 🎯 扩展模板

当需要处理新的数据表时，请按以下模板创建文件：

```
# 脚本文件
etl_dataclean_{新数据源}_{新表类型}.py

# 配置文件  
config_{新数据源}_{新表类型}.yaml
...
**未来规划：** 分层配置架构（10+表时考虑）
- `config_base.yaml` - 共享配置（日志、输出、增量处理）
- `config_{数据源}_{表类型}.yaml` - 表特定配置（数据源路径、字段映射）
